<?php
namespace Lockminds\Teams\Classes;

use Lockminds\Teams\Classes\Metronic;
...
class Init
{
    ...
    {
        if (!empty(config('lm_team_layouts.page-loader.type'))) {
    ...
    {
        if (config('lm_team_layouts.header.self.fixed.desktop')) {
        ...
        if (config('lm_team_layouts.header.self.fixed.mobile')) {
        ...
        if (config('lm_team_layouts.header.menu.self.display')) {
            Metronic::addClass('header_menu', 'header-menu-layout-' . config('lm_team_layouts.header.menu.self.layout'));

            if (config('lm_team_layouts.header.menu.self.root-arrow')) {
                Metronic::addClass('header_menu', 'header-menu-root-arrow');
            }
        }

        if (config('lm_team_layouts.header.self.width') === 'fluid') {
    ...
    {
        if (config('taskmanager.lm_team_layouts.subheader.display')) {
            Metronic::addClass('body', 'subheader-enabled');
        } else {
            return;
        }

        $subheader_style = config('taskmanager.lm_team_layouts.subheader.style');
        $subheader_fixed = config('taskmanager.lm_team_layouts.subheader.fixed');

        // Fixed content head
        if (config('taskmanager.lm_team_layouts.subheader.fixed') && config('taskmanager.lm_team_layouts.header.self.fixed.desktop')) {
        ...
        if (config('lm_team_layouts.subheader.width') == 'fluid') {
            Metronic::addClass('subheader-container', 'container-fluid');
        } else {
            Metronic::addClass('subheader-container', 'container');
        }

        if (config('taskmanager.lm_team_layouts.subheader.clear')) {
    ...
    {
        if (config('taskmanager.lm_team_layouts.content.fit-top')) {
            Metronic::addClass('content', 'pt-0');
        }

        if (config('taskmanager.lm_team_layouts.content.fit-bottom')) {
            Metronic::addClass('content', 'pt-0');
        }

        if (config('taskmanager.lm_team_layouts.content.width') == 'fluid') {
    ...
    {
        if (config('taskmanager.lm_team_layouts.aside.self.display') != true) {
            return;
        }
        ...
        // Fixed Aside
        if (config('taskmanager.lm_team_layouts.aside.self.fixed')) {
        ...
        // Default fixed
        if (config('taskmanager.lm_team_layouts.aside.self.minimize.default')) {
            Metronic::addClass('body', 'aside-minimize');
        }

        // Menu
        // Dropdown Submenu
        if (config('taskmanager.lm_team_layouts.aside.menu.dropdown') == true) {
        ...
        if (config('taskmanager.lm_team_layouts.aside.menu.submenu.dropdown.hover-timeout')) {
            Metronic::addAttr('aside_menu', 'data-menu-dropdown-timeout', config('taskmanager.lm_team_layouts.aside.menu.submenu.dropdown.hover-timeout'));
        }
    }
    ...
    {
        // Fixed header
        if (config('taskmanager.lm_team_layouts.footer.fixed') == true) {
            Metronic::addClass('body', 'footer-fixed');
        }

        if (config('taskmanager.lm_team_layouts.footer.width') == 'fluid') {
